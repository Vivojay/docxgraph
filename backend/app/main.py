...
from .deps import get_current_user_api, get_db, require_admin_api, require_auditor_api
...
from .retrieval import get_case_tags_map, match_experts, refresh_case_similarity_edges, similar_cases_for_case
from .schemas import (
    AuditLogOut,
    AuthLoginRequest,
    AuthLoginResponse,
    ...
    return HTMLResponse(
        ...
        <html>
          <body style="font-family: sans-serif; max-width: 720px; margin: 40px auto;">
            <h1>ExperienceGraph Enterprise</h1>
            <p>The supported product UI is the React frontend at <a href="{settings.frontend_url}">{settings.frontend_url}</a>.</p>
            <p>This backend remains available for API access, docs, and local fallback workflows.</p>
            <p>Demo credentials: admin@demo.health / AdminPass123!</p>
...
def api_search_cases(payload: SearchRequest, user: User = Depends(get_current_user_api), db: Session = Depends(get_db)):
    from .retrieval import find_similar_cases
